    ...
    class TasksDefaultController extends ActivityModelsDefaultController
    {
        ...
        */
        ...
        protected function attemptToSaveModelFromPost($model, $redirectUrlParams = null, $redirect = true, $returnOnValidate = false)
        {
            ...
            {
                $linkUrl = Yii::app()->getBaseUrl(true).'/index.php/tasks/default/details?id='.$model->id;
                $subject = '[VERTWARE] A NEW TASK IS ASSIGNED TO YOU';
                $redirectUrlParams = '';
                if($model->owner->id != $oldOwnerId)
                {
                    ...
                    $mailContent = array(
                        'subject' => $subject,
                        'content' => 'Hi '.$ownerAccount->firstName .', <br> <p> <b>Task Details:</b> <br> Name: '.$model->name.'<br> Assigned by: '.$accountAssigner->getFullName().' <br> <br> Link: <a href="'.$linkUrl.'">'.$linkUrl.'</a></p> 
