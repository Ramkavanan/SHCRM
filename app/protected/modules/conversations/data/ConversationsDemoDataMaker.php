<?php
    ...
    class ConversationsDemoDataMaker extends DemoDataMaker
    {
        ...
        // For the importing
        public function makeImport(& $demoDataHelper, $file_name)
        {
            ...
            $conversations = array();
            if($file_name == 'lead_activity')
            {
                $model_class_name   = 'Contact';
                $method_name        = 'getByOldLeadId';
            }
            elseif($file_name == 'account_activity')
            {
                $model_class_name   = 'Account';
                $method_name        = 'getByOldAccountId';
            }
            elseif($file_name == 'contact_activity')
            {
                $model_class_name   = 'Contact';
                $method_name        = 'getByOldContactId';
            }
            
            $csvFile        = Yii::app()->basePath.'/csv_files/'.$file_name.'.csv';
           ...
           // $ran = array('super','test','dbakh','darroldenglish'); 
            
           // print_r($line_of_text);
           // die;
            
            unset($line_of_text[0]); // coz first is the header
                        
            foreach ($line_of_text as $importData)
            {
                if(is_array($importData))
                {
                    $importDataSan = array_filter($importData);
                    
                    print_r($importDataSan);
                    
                   // echo date("Y-m-d H:i:s", strtotime($importDataSan['2']));
                   // die;

                    if(count($importDataSan)>3)
                    {
                       // $importDataSan['1'] = $ran[array_rand($ran, 1)];
                        
                        $postData                       = array();
                        $conversation                   = new Conversation();
                        $conversation->setScenario('importModel');
                        $conversation->owner            = User::getByUsername($importDataSan['1']);
                        $conversation->createdByUser    = $conversation->owner;                        
                        $conversation->subject->value   = $importDataSan['3'];
                        $conversation->description      = $importDataSan['6'];
                        
                        $return_data_arr = $model_class_name::$method_name($importDataSan['0']);
                        if(isset($return_data_arr[0]))
                        { 
                            //print_r($return_data_arr);
                            $conversation->conversationItems->add($return_data_arr[0]);
                            ...
                            $conv_update                  = Conversation::GetById(intval($conversation->id));
                            $conv_update->latestDateTime  = date("Y-m-d H:i:s", strtotime($importDataSan['2']));
                            $conv_update->save();
                            
                            $conversations_leads[] = $importDataSan['0'];
