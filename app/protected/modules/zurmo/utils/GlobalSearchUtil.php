    ...
    class GlobalSearchUtil
    {
        ...
        protected static function findGlobalSearchScopingModuleNamesAndLabelsDataByUser(User $user)
        {
            ...
            foreach ($modules as $module)
            {
                ...
                {
                    if(!empty(Yii::app()->user->userModel->isRootUser)){    // To check super user or normal user.
                        if(($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                            $moduleNamesAndLabels[$module->getName()] = $module->getModuleLabelByTypeAndLanguage('Plural');    // For super user global search.
                        }
                    }else{
                        $userIsInAccountManGroup = CostbookUtils::GetIsAccountManagerGroup();    //Check if user in Account manager group.
                        $userIsInCatalogManGroup = CostbookUtils::GetIsCatalogManagerGroup();    //Check if user in Catalog manager group.
                        if(($userIsInAccountManGroup && $userIsInCatalogManGroup && Yii::app()->user->userModel->role->name == Constant::GM) || ($userIsInAccountManGroup && $userIsInCatalogManGroup)){
                            if(($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                       ...
                       /**
                         * Search result for users in Catalog manager group users and in GM role.
                         */
                        if($userIsInCatalogManGroup && Yii::app()->user->userModel->role->name == Constant::GM){
                            if(($module->getName() != 'departmentReferences') && ($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                                $moduleNamesAndLabels[$module->getName()] = $module::getModuleLabelByTypeAndLanguage('Plural');
                            }                            
                        }
                        
                       /**
                         * Search result for users in Account manager group and GM role.
                         * Search result for user only in GM role.
                         */
                        if(($userIsInAccountManGroup && Yii::app()->user->userModel->role->name == Constant::GM) || ($userIsInAccountManGroup)){
                            if(($module->getName() != 'costbook') && ($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                                $moduleNamesAndLabels[$module->getName()] = $module::getModuleLabelByTypeAndLanguage('Plural');
                            }                            
                        }    
                        
                       /**
                         * Search result for users only in Catalog manager group.
                         */
                        if($userIsInCatalogManGroup){
                            if(($module->getName() != 'departmentReferences') && ($module->getName() !='jobScheduling') && ($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                                $moduleNamesAndLabels[$module->getName()] = $module::getModuleLabelByTypeAndLanguage('Plural');
                            }                            
                        }
                        
                       /**
                         * Search result for Normal users.
                         */
                        if((!$userIsInAccountManGroup) && (!$userIsInCatalogManGroup) && (Yii::app()->user->userModel->role->name != Constant::GM)){
                            if(($module->getName() != 'costbook') && ($module->getName() != 'categories') && ($module->getName() != 'unitofmeasures') && ($module->getName() != 'departmentReferences') && ($module->getName() !='jobScheduling') && ($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                                $moduleNamesAndLabels[$module->getName()] = $module::getModuleLabelByTypeAndLanguage('Plural');
                            }                            
                        }
                        
                       /**
                         * Search result for only in GM role.
                         */
                        if(Yii::app()->user->userModel->role->name == Constant::GM){
                            if(($module->getName() != 'costbook') && ($module->getName() != 'categories') && ($module->getName() != 'unitofmeasures') && ($module->getName() != 'departmentReferences') && ($module->getName() != 'projects') && ($module->getName() != 'products') && ($module->getName() != 'marketing')){
                                $moduleNamesAndLabels[$module->getName()] = $module::getModuleLabelByTypeAndLanguage('Plural');
                            }                            
                        }
                    }
