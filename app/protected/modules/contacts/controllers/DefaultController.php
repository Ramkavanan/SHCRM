<?php
    ...
    class ContactsDefaultController extends ZurmoModuleController
    {
        ...
        public function actionCreateFromRelation($relationAttributeName, $relationModelId, $relationModuleId, $redirectUrl, $isPrimaryContact=null)
        {
            ...
            $this->actionCreateByModel($contact, $redirectUrl, $isPrimaryContact);
        }

        protected function actionCreateByModel(Contact $contact, $redirectUrl = null, $isPrimaryContact = null)
        {
            $requestFrom = 'from_create';
            $titleBarAndEditView = $this->makeEditAndDetailsView(
                                            $this->attemptToSaveModelFromPost($contact, $redirectUrl, $requestFrom, $isPrimaryContact), 'Edit');
            $view = new ContactsPageView(ZurmoDefaultViewUtil::
        ...
        {
            $requestFrom = 'from_edit';
            $view    = new ContactsPageView(ZurmoDefaultViewUtil::
                            makeStandardViewForCurrentUser($this,
                            $this->makeEditAndDetailsView(
                                $this->attemptToSaveModelFromPost($contact, $redirectUrl, $requestFrom), 'Edit')));
            echo $view->render();
        }
        ...
        {
            $printData = '';
            $printView =   new ContactPrintView($printData, $id);
         ...
         */
        protected function attemptToSaveModelFromPost($model, $redirectUrlParams = null, $requestFrom = null, $isPrimaryContact= null, $redirect = true, $returnOnValidate = false)
        {
            ...
            {
                if(!$isPrimaryContact){
                    $model->isPrimaryContact = FALSE;
                    $model->save();
                }
                $linkUrl = Yii::app()->getBaseUrl(true).'/index.php/contacts/default/details?id='.$model->id;
                if(is_string($requestFrom))
                {
                    if($requestFrom == 'from_create')
                    {
                        $subject = '[VERTWARE] A NEW CONTACT IS ASSIGNED TO YOU';
                    }elseif ($requestFrom == 'from_edit') {
                        $subject = '[VERTWARE] CONTACT OWNER CHANGE';
                    }
                        if($model->owner->id != $oldOwnerId)
                        {
                            ...
                            $contactAssigner     = User::getById(Yii::app()->user->id);
                            if (!empty($contactAssigner->primaryEmail->emailAddress)){
                                $fromAddress = $contactAssigner->primaryEmail->emailAddress;
                            } else {                                   
                            ...
                            $mailContent = array(
                                'subject' => $subject,
                                'content' => 'Hi '.$ownerAccount->firstName .', <br> <p> <b>Contact Details:</b> <br> Name: '.$model->firstName.' '.$model->lastName.'<br> Assigned by: '.$contactAssigner->getFullName().' <br> <br> Link: <a href="'.$linkUrl.'">'.$linkUrl.'</a></p> 
