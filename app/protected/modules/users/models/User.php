    ...
    class User extends Permitable
    {
        ...
        protected function beforeSave()
        {
            if(Yii::app()->controller->action->id == 'create' || Yii::app()->controller->action->id == 'edit'){
                $errMsg = array();
                if (empty($this->modelClassNameToBean['Person']->jobtitle)){
                    //$this->addError('jobTitle', Zurmo::t('Core', 'Job title cannot be blank.'));
                   // return false;
                    $errMsg['jobTitle'] = Zurmo::t('Core', 'Job Title cannot be blank.');
                }
                if (empty($this->modelClassNameToBean['Person']->officephone)){
                    //$this->addError('officePhone', Zurmo::t('Core', 'Office Phone cannot be blank.'));
                   // return false;
                    $errMsg['officePhone'] = Zurmo::t('Core', 'Office Phone cannot be blank.');
                }
                if (($this->manager->id < 0)){
                    //$this->addError('manager', Zurmo::t('Core', 'Manager cannot be blank.'));                
                    //return false;
                    $errMsg['manager'] = Zurmo::t('Core', 'Manager cannot be blank.');
                }
                if (($this->role->id < 0)){
                    // $this->addError('role', Zurmo::t('Core', 'Role cannot be blank.'));
                    //return false;
                    $errMsg['role'] = Zurmo::t('Core', 'Role cannot be blank.');
                }
                if (empty($this->modelClassNameToBean['Person']->mobilephone)){
                    //$this->addError('mobilePhone', Zurmo::t('Core', 'Mobile Phone cannot be blank.'));
                    //return false;
                    $errMsg['mobilePhone'] = Zurmo::t('Core', 'Mobile Phone cannot be blank.');
                }

                if (empty($this->modelClassNameToBean['Person']->department)){
                    //$this->addError('department', Zurmo::t('Core', 'Department cannot be blank.'));
                    //return false;
                    $errMsg['department'] = Zurmo::t('Core', 'Department cannot be blank.');
                }
                if (empty($this->primaryEmail) || $this->primaryEmail == '(None)'){
                    $this->primaryEmail->addError('emailAddress', Zurmo::t('Core', 'Email cannot be blank.'));
                    //return false;
                    $errMsg['emailAddress'] = Zurmo::t('Core', 'Email cannot be blank.');
                }            
                $isAdderss = FALSE;
                if(!empty($this->primaryAddress->street1) || !empty($this->primaryAddress->street2) || !empty($this->primaryAddress->city) || !empty($this->primaryAddress->city) || !empty($this->primaryAddress->state) || !empty($this->primaryAddress->postalcode) || !empty($this->primaryAddress->country)){
                    $isAdderss = TRUE;
                }
                if(!$isAdderss){
                    $this->primaryAddress->addError('street1', Zurmo::t('Core', 'Address cannot be blank.'));
                    //return false;
                    $errMsg['primaryAddress'] = Zurmo::t('Core', 'Address cannot be blank.');
                }            
                if (empty($this->modelClassNameToBean['User']->language)){
                    //$this->addError('language', Zurmo::t('Core', 'Language cannot be blank.'));
                    //return false;
                    $errMsg['language'] = Zurmo::t('Core', 'Language cannot be blank.');
                }            
                if (empty($this->modelClassNameToBean['User']->locale)){
                    //$this->addError('locale', Zurmo::t('Core', 'Locale cannot be blank.'));
                    //return false;
                    $errMsg['locale'] = Zurmo::t('Core', 'Locale cannot be blank.');
                }            
                if (empty($this->modelClassNameToBean['User']->timezone)){
                    //$this->addError('timeZone', Zurmo::t('Core', 'TimeZone cannot be blank.'));
                    //return false;
                    $errMsg['timeZone'] = Zurmo::t('Core', 'TimeZone cannot be blank.');
                }            
                if ($this->currency->id <0){
                    //$this->addError('currency', Zurmo::t('Core', 'Currency cannot be blank.'));
                    //return false;
                    $errMsg['currency'] = Zurmo::t('Core', 'Currency cannot be blank.');
                }            
                if(count($errMsg) > 0)
                {
        ...
        public static function getDefaultMetadata()
        {
            ...
            $metadata[__CLASS__] = array(
                ...
                'rules' => array(
                    ...
                    array('outlookEmail',   'email'),
                    array('accessToken',    'type',    'type' => 'string'),
                    array('refreshtoken',    'type',    'type' => 'string'),
                    array('isOutlookSynced',       'type',      'type' => 'boolean'),
                ),
                'elements' => array(
        ...
        */
        ...
        // To get the user details based on the role: GM
        public static function getEmailByGmRole()
        {
            $roleObj = Role::getByName(User::GM);
            return self::makeModels(ZurmoRedBean::find('_user', "role_id = :role_id ", array(':role_id' => $roleObj->id)));
        }
        
        public static function getUsersByEstimatorRole($partialName='')
        {        
            $estimatorGroupDetails = Group::getByName(User::Estimator, 'fromAutoComplete');
            $userIdArr = array();
            foreach($estimatorGroupDetails->users as $estimatorGroupUserId)
            {
                $userIdArr[] = $estimatorGroupUserId->id;
            }
           // $userIdArr = array();
            ...
            $fullNameSql = DatabaseCompatibilityUtil::concat(array('person.firstname',
             ...
             $where .= "      (person.firstname      like lower('$partialName%') or "    .
                      ...
                      "       $fullNameSql like lower('$partialName%')) AND isactive = 1 AND _user.id IN (".implode(',', $userIdArr).")";
             
            return self::getSubset($joinTablesAdapter, null, 10,
                                            $where, "person.firstname, person.lastname");
        }
        
        public static function getUsersByGmGroup($partialName='')
        {        
            $estimatorGroupDetails = Group::getByName(User::ACCOUNTMANAGER, 'fromAutoComplete');
            $userIdArr = array();
            foreach($estimatorGroupDetails->users as $estimatorGroupUserId)
            {
                $userIdArr[] = $estimatorGroupUserId->id;
        ...
        {        
            $query = "SELECT E.id, P.*, U.* FROM email as E 
                      LEFT JOIN person AS P ON P.primaryemail_email_id = E.id
                      LEFT JOIN _user AS U ON U.person_id = P.id
                    WHERE emailaddress = '$email'";
            
            return ZurmoRedBean::getRow($query);            
        }
        
        public function getUserByHashEmail($email, $hash)
        {        
            $query = "SELECT U.id FROM email as E
                      LEFT JOIN person AS P ON P.primaryemail_email_id = E.id
                      LEFT JOIN _user AS U ON U.person_id = P.id AND hash = '$hash'
                    WHERE md5(emailaddress) = '$email'";
            
            return ZurmoRedBean::getRow($query);
        }
        
        
        public function savePassword($runValidation = true, array $attributeNames = null)
        {
            $passwordChanged = array_key_exists('hash', $this->originalAttributeValues);
            unset($this->originalAttributeValues['hash']);
            assert('!isset($this->originalAttributeValues["hash"])');
        ...
        public function saveOutlookDetails($outlookEmail, $accessToken, $id, $refreshToken)
        {        
            $query = "update _user set outlookemail = '$outlookEmail', accesstoken = '$accessToken', refreshtoken = '$refreshToken', isOutlookSynced = 1
                    WHERE id = ".$id;
        ...
        /**
         * Get list of user by given classname
         * @param $classname
         * @param $password
         * @return An
         * @author Sundar P - 08-Sep-2016         
         */
        public function getUsersByClassname($className='ZurmoModule'){        
            $query = "SELECT serializedmetadata,id,_user_id FROM `perusermetadata` WHERE classname='$className';";
