    ...
    class UsersDefaultController extends ZurmoModuleController
    {
        ...
        public function filters()
        {
            ...
            $filters[] = array(
                    ...
                    ' - modalList, - switchTo, autoComplete, autoCompleteGm, details, profile, edit, auditEventsModalList, changePassword, ' .
                    'configurationEdit, applyListCount, emailConfiguration, securityDetails, notificationConfiguration, ' .
                    'autoCompleteForMultiSelectAutoComplete, confirmTimeZone, changeAvatar, gameDashboard, forgotPassword, resetPassword, logAuditEvent, addOutlook',
        ...
        public function actionAddOutlook($code) {
            ...
            if(isset($code)){
                ...
                $tokens = outlookCalendar::getTokenFromAuthCode("authorization_code", $authCode, $redirectUri);
                if ($tokens['access_token']) {
                    ...
                    $outlookUser = outlookCalendar::getUser($tokens['access_token']);
                    
                    if(isset($outlookUser['EmailAddress']))
                    {
                        $outlookEmail = $outlookUser['EmailAddress'];
                        $accessToken  = $tokens['access_token'];
                        $isOutlookSynced = true;
                        User::saveOutlookDetails($outlookEmail, $accessToken, $id, $tokens['refresh_token']);
                        Yii::app()->user->setFlash('notification',
                            Zurmo::t('UsersModule', 'Outlook calendar configuration saved successfully.')
                        );
                    }
                    else
                    {
                        Yii::app()->user->setFlash('notification',
                            Zurmo::t('UsersModule', 'Problem in outlook calendar configuration. Please try after some time.')
                        );
                    }
        ...
        public function actionModalList()
        {
            ...
            if($_GET['modalTransferInformation']['sourceIdFieldId'] == 'Opportunity_estimator_id' || $_GET['modalTransferInformation']['sourceIdFieldId'] == 'ApprovalProcess_actualapprover_id')
            {
                echo UserSearchUtil::setAjaxModeAndRenderModalSearchList($this, $modalListLinkProvider,
                                                'SelectEstimatorTypeUsersMetadataAdapter');
            }
            else if($_GET['modalTransferInformation']['sourceIdFieldId'] == 'Opportunity_gm_id' || $_GET['modalTransferInformation']['sourceIdFieldId'] == 'ApprovalProcess_actualgmapprover_id')
            {
                echo UserSearchUtil::setAjaxModeAndRenderModalSearchList($this, $modalListLinkProvider,
                                                'SelectGmTypeUsersMetadataAdapter');
            }
            else
            {
        ...
        /**
         * Update listing page count
         * @param $id
         * @return Null
         * @author Sundar P - 08-Sep-2016         
         */
        public function actionApplyListCount($id=100)
        {
            $allUsers = User::getUsersByClassname('ZurmoModule');
            //$allUsers = User::getActiveUsers();
            for($v=0; $v<count($allUsers); $v++){
                $srlzeusrid = $allUsers[$v]['_user_id'];
                $user = User::getById(intval($srlzeusrid));                             
                $configurationForm = UserConfigurationFormAdapter::makeFormFromUserConfigurationByUser($user);
                $configurationForm->listPageSize = (int)$id;
                UserConfigurationFormAdapter::setConfigurationFromFormForCurrentUser($configurationForm);

            }
            Yii::app()->user->setFlash('notification',
                        Zurmo::t('UsersModule', 'List count saved successfully.')
                    );
            $this->redirect(array($this->getId() . '/profile'));
        ...
        public function actionAutocompleteEstimator()
        {
            $term = trim($_GET['term']);

            if($term !='')
            {
                $users =  User::getUsersByEstimatorRole($term);
                $autoCompleteResults  = array();
                foreach ($users as $user)
                {
                    $autoCompleteResults[] = array(
        ...
        public function actionAutocompleteGm()
        {
            $term = trim($_GET['term']);

            if($term !='')
            {
                $users =  User::getUsersByGmGroup($term);
                $autoCompleteResults  = array();
                foreach ($users as $user)
                {
                    $autoCompleteResults[] = array(
        ...
        {            
            ...
            {            
                ...
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                  $this->redirect(Yii::app()->baseUrl . '/index.php/users/default/forgotPassword?err=1');
                }
                else
                {
                    ...
                    if(isset($user['hash']))
                    {                    
                        $linkUrl = Yii::app()->getBaseUrl(true) . '/index.php/users/default/resetPassword?hash='.$user['hash'].'&email='.md5($email);
                        ...
                        $this->sendEMail($fromAddress, $email, $mailContent);
                        Yii::app()->user->setFlash('notification',
                            Zurmo::t('UsersModule', 'Password reset link sent to your mail. Kindly check it.')
                        );
                        $this->redirect(Yii::app()->baseUrl . '/index.php/zurmo/default/login');
                    }
                    else {                        
                        $this->redirect(Yii::app()->baseUrl . '/index.php/users/default/forgotPassword?err=2');
                    }
                }
            }
            
            $forgotPasswordView =   new ForgotPasswordPageView();
            echo $forgotPasswordView->render();
        }
        
        //Generate E-Mail For Opportunity and Agreement
        public function sendEMail($fromAddress, $recipients, $content)
        {
            $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= "From: VERTWARE<notifications@vertware.net>" . "\r\n";
            mail($recipients, $content['subject'], $content['content'], $headers);
            return true;                              
        }
        ...
        {            
            ...
            if(isset($_POST['UserPasswordForm']['newPassword']))
            {            
                $postVariableName = 'UserPasswordForm';
                if (isset($_POST[$postVariableName]))
                {
                    $postData   = $_POST[$postVariableName];
                    $userDetails = User::getUserByHashEmail($_SESSION['email'], $_SESSION['hash']);
                    
                    if (!$userDetails['id'])
                    {
                        $this->redirect(Yii::app()->baseUrl . '/index.php/users/default/resetPassword?hash='.$_SESSION['hash'].'&email='.$_SESSION['email'].'&err=1');
                    }
                    
                    $user = User::getById(intval($userDetails['id']));
                    ...
                    Yii::app()->user->setFlash('notification',
                        Zurmo::t('UsersModule', 'Password have been reset successfully.')
                    );
                    unset($_SESSION['hash']);
            ...
            {
                Yii::app()->user->setFlash('notification',
                    Zurmo::t('UsersModule', 'Password reset link seems to be expired. Please generate a new one.')
                );
                $this->redirect(Yii::app()->baseUrl . '/index.php/zurmo/default/login');
            }
            
            $resetPasswordView =   new ResetPasswordPageView();
            echo $resetPasswordView->render();
        }
    }
?>
