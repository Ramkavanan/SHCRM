<?php
    ...
    class OpportunitiesByStageChartDataProvider extends ChartDataProvider
    {
        ...
        public function getChartData()
        {
            ...
            $newChartData[Constant::CONSULTING] = $chartData[Constant::CONSULTING];
            $newChartData[Constant::ESTIMATE] = $chartData[Constant::ESTIMATE];
            $newChartData[Constant::FINALPROPOSAL] = $chartData[Constant::FINALPROPOSAL];
            $newChartData[Constant::AGREEMENT] = $chartData[Constant::AGREEMENT];
            $newChartData[Constant::WON] = $chartData[Constant::WON];
            $newChartData[Constant::LOST] = $chartData[Constant::LOST];
        ...
        protected static function makeChartSqlQuery()
        {
            ...
            // Commented and added to get the currency value for the final amount
            //$joinTablesAdapter->addFromTableAndGetAliasName('currencyvalue', 'amount_currencyvalue_id', 'opportunity');
            $joinTablesAdapter->addFromTableAndGetAliasName('currencyvalue', 'finalamount_currencyvalue_id', 'opportunity');
            
            // Added To get the data of the current year
            $joinTablesAdapter->addFromTableAndGetAliasName('ownedsecurableitem', 'ownedsecurableitem_id', 'opportunity');
            $joinTablesAdapter->addFromTableAndGetAliasName('securableitem', 'securableitem_id', 'ownedsecurableitem');
            $joinTablesAdapter->addFromTableAndGetAliasName('item', 'item_id', 'securableitem');
            $where = 'opportunity.archive IS NULL AND YEAR(item.createddatetime) = YEAR(CURDATE())';
            // Ends Here
