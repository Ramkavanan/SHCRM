<?php
    ...
    class ReportsDefaultController extends ZurmoBaseController
    {
        ...
        public function actionDetails($id,$reportYear='')
        {
            $savedReport = static::getModelAndCatchNotFoundAndDisplayError('SavedReport', intval($id));
           
            // To load the Agrmt Repors for the Jobschedule
            if(trim($savedReport->name) == "Job schedule Report- Agreement")
            {                
                if($reportYear)
                    $reportDate  = $reportYear."-01-01";
                else
                    $reportDate  = date("Y-01-01");
                $weekArr    = JobSchedulingUtils::getDateByWeek($reportDate);
                $dateWeek   = array_flip($weekArr);
                
                $view       = new JobReportView($dateWeek, 'Agreement', $reportDate);
                $jobView    = new JobPageView(ZurmoDefaultViewUtil::makeStandardViewForCurrentUser($this, $view));
                echo $jobView->render();  
            }
            else if(trim($savedReport->name) == "Job schedule Report - Category")
            {                
                if($reportYear)
                    $reportDate  = $reportYear."-01-01";
                else
                ...
                $view       = new JobReportView($dateWeek, 'Category', $reportDate);
                $jobView    = new JobPageView(ZurmoDefaultViewUtil::makeStandardViewForCurrentUser($this, $view));
                echo $jobView->render();
            }
            else if(trim($savedReport->name) == "Job schedule Report - Account Owner")
            {                
                if($reportYear)
                    $reportDate  = $reportYear."-01-01";
                else
                ...
                $view       = new JobReportView($dateWeek, 'Account', $reportDate);
                $jobView    = new JobPageView(ZurmoDefaultViewUtil::makeStandardViewForCurrentUser($this, $view));
                echo $jobView->render();
            }
            else
