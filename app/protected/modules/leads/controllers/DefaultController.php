<?php
    ...
    class LeadsDefaultController extends ZurmoModuleController
    {
        ...
        public function actionCreate()
        {
            ...
            $titleBarAndEditView = $this->makeEditAndDetailsView(
                    $this->attemptToSaveModelFromPost(new Contact(),'from_create'), 'Edit',
	...
	public function actionPrintView($id)
        {
            $printData = '';   
            $printView =   new LeadsPrintView($printData, $id);
           // $formattedPrintView =  new MyLeadsPrintView(ZurmoDefaultViewUtil::makeStandardViewForCurrentUser($this, $printView));
            echo $printView->render();            
        }
        
        // Added to send the email to the new assigned user
        protected function attemptToSaveModelFromPost($model, $redirectUrlParams = null, $redirect = true, $returnOnValidate = false)
        {
            ...
            {
                $linkUrl = Yii::app()->getBaseUrl(true).'/index.php/leads/default/details?id='.$model->id;
                if(is_string($redirectUrlParams))
                {
                    if($redirectUrlParams == 'from_create')
                    {
                        $subject = '[VERTWARE] A NEW LEAD IS ASSIGNED TO YOU';
                    }elseif ($redirectUrlParams == 'from_edit') {
                        $subject = '[VERTWARE] LEAD OWNER CHANGE';
                    }
                        $redirectUrlParams = '';
                        if($model->owner->id != $oldOwnerId)
                        {
                            $leadAssigner     = User::getById(Yii::app()->user->id);
                            $model->assignedBy =  $leadAssigner;
                            $model->assignedDateCstm =  date("Y-m-d");
                            $model->save();
                            ...
                            if (!empty($leadAssigner->primaryEmail->emailAddress)){
                                $fromAddress = $leadAssigner->primaryEmail->emailAddress;
                            } else {                                   
