 ...
 */
class AgreementTrackingCustomView extends View {
...
//                    Yii::getPathOfAlias('application.modules.agreementProducts.elements.assets')) . '/AgreementProductTemplateUtils.js');
       ...
       // Yii::app()->clientScript->registerScriptFile(Yii::app()->getAssetManager()->publish(
                     //   Yii::getPathOfAlias('application.modules.opportunityProducts.elements.assets')) . '/AgreementTrackingTemplateUtils.js', CClientScript::POS_END);
    }
    ...
    protected function renderXHtmlHead() {
        ...
        if (file_exists("themes/$themeName/ico/favicon.png")) {
            $cs->registerLinkTag('shortcut icon', null, $themeBaseUrl . '/ico/favicon.png');
        } else {
            $cs->registerLinkTag('shortcut icon', null, $defaultThemeBaseUrl . '/ico/favicon.png');
        }
        return '<head>' .
                '<meta charset="utf-8">' .
    ...
    public function appendContentBeforeXHtmlBodyEndAndXHtmlEnd($content, $additionalContent) {
        $content = str_replace($this->renderXHtmlBodyEnd() . $this->renderXHtmlEnd(), $additionalContent . $this->renderXHtmlBodyEnd() . $this->renderXHtmlEnd(), $content);
        return $content;
    }
