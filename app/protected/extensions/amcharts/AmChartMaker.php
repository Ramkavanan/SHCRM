<?php
    ...
    class AmChartMaker
    {
        ...
        private function addChartPropertiesByType()
        {
            ...
            elseif ($this->type == ChartRules::TYPE_GICRM_PIE_2D)
            {
                $this->addChartProperties('outlineColor',           "'#FFFFFF'");
                $this->addChartProperties('outlineAlpha',           0.8);
                $this->addChartProperties('outlineThickness',       1);
                $this->addChartProperties('labelTickColor',         "'#000000'");
                $this->addChartProperties('pullOutRadius',          "'2%'");
                $this->addChartProperties('startDuration',          0);
                $this->addChartProperties('labelText',            "'[[title]]: " . $this->yAxisUnitContent . "[[percents]]%'");
                $this->addValueAxisProperties('strictMinMax',       true);
                $this->addValueAxisProperties('minimum',            100);
        ...
        public function javascriptChart()
        {
            ...
            //For company overview dashboard
            if($this->type == 'StackedColumn2D')
            {
                $javascript .="
                        var chart           = new AmCharts.AmSerialChart();
                        chart.dataProvider  = chartData_{$this->id};
